...
class ChatCompletionRequest(BaseModel):
    ...
    @classmethod
    def last_message_must_be_user_or_system(cls, v: list[ChatMessage]) -> list[ChatMessage]:
        if not v:
            raise ValueError("messages list cannot be empty")
        if v[-1].role == "assistant":
            raise ValueError("the last message must be from 'user' or 'system', not 'assistant'")
        return v
